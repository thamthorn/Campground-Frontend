    ...
    try {
      if (date) {
        ...
        const response = await axios.post(
          `${config.api}/campgrounds/${params.cid}/booking`,
          payload,
          config.headers()
        );
      ...
      if (
        error.response.data.message === "Not authorize to access this route"
      ) {
        Swal.fire({
        ...
        <div className="mt-[10vh] p-0 m-0 w-screen h-[90vh]">
          ...
          <div className="flex flex-row px-12 place-content-center">
            <div className="w-[30%] md:h-auto text-green-900 px-5 text-left">
              <div>
                <h2 className="text-[18px] text-green-900">
                  {campground.name}
                </h2>
                <div className="text-[16px]  mt-8">
                  {"Adress: " + campground.address}
                </div>
                <div className="text-[16px]">
                  <LocalPhoneIcon /> {campground.tel}
                </div>
                <div className="text-[16px]">
                ...
                <div className="flex items-center text-[16px]">
                  ...
                  <Rating
                    className="my-1"
                    name="read-only"
                    value={parseFloat(campground.rating.toString())}
                    readOnly
                  />
                </div>
              </div>
              ...
              <form className="card-body">
                ...
                <div className="form-control mt-6">
                  <button
                    className="hover:bg-green-900 hover:text-white text-green-900 py-1 px-4 border border-gray-400 rounded-md"
                    type="button"
